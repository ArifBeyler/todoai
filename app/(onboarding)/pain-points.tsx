...
const CHALLENGES: ChallengeOption[] = [
  { value: "procrastination", label: "Erteleme", icon: Hourglass },
  { value: "focus", label: "Odaklanma", icon: Eye },
  { value: "motivation", label: "Motivasyon Eksikliği", icon: BatteryLow },
  { value: "time_management", label: "Zaman Yönetimi", icon: ClockCounterClockwise },
  { value: "stress", label: "Stres", icon: Lightning },
];

export default function PainPointsScreen() {
  const { challenges, setChallenges } = useSessionStore();
  ...
  const handleContinue = () => {
    ...
    triggerExit("forward", () => router.push("/(onboarding)/social-proof"));
  };
  ...
  const titleStr = "En çok zorlandığın\nşey ne?";
  const subStr =
    "Birden fazla seçebilirsin. Sana özel çözümler sunacağız.";
  ...
  return (
    <Animated.View style={[styles.container, exitStyle]}>
      <View style={[styles.panel, CARD_SHADOW]}>
        <OnboardingProgress current={3} total={11} />
...
type ChallengeCardProps = {
  item: ChallengeOption;
  index: number;
  isSelected: boolean;
  onToggle: () => void;
};

const ChallengeCard = ({ item, index, isSelected, onToggle }: ChallengeCardProps) => {
